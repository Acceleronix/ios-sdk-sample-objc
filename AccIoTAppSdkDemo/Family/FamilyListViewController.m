...
- (void)addButtonClick {
    UIAlertController *alertVc = [UIAlertController alertControllerWithTitle:@"Create Family" message:nil preferredStyle:UIAlertControllerStyleAlert];
    ...
    [alertVc addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = @"Please enter the family name";
    }];
...
- (void)addFamily:(UIAlertController *)alert {
    ...
    if (familyName.length == 0) {
        NSLog(@"Please enter the family name");
        return;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [btn setTitle:@"Family Details" forState:UIControlStateNormal];
