...
- (void)createUI {
    ...
    self.nameTextField.placeholder = @"Please enter the scene name";
    ...
    [addBtn setTitle:@"Add Device" forState:UIControlStateNormal];
...
- (void)createButtonClick {
    if (self.nameTextField.text.length == 0) {
        [self.view makeToast:@"Please enter the scene name" duration:1 position:CSToastPositionCenter];
        return;
    }
    
    if (self.dataArray.count == 0) {
        [self.view makeToast:@"Please select a device" duration:1 position:CSToastPositionCenter];
        return;
    }
