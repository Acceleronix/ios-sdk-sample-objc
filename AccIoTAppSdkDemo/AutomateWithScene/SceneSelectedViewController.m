...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isScene && (model.isShared || model.bindMode == 1 || model.capabilitiesBitmask == 4)) {
        ...
        [self.view makeToast:@"Multi-binding devices, shared devices, and pure Bluetooth devices do not support adding to scenes" duration:1 position:CSToastPositionCenter];
        return;
    }
    
    if (self.isAutomate && (model.isShared || model.bindMode == 1 || model.capabilitiesBitmask == 4)) {
        [tableView deselectRowAtIndexPath:indexPath animated:YES];
        [self.view makeToast:@"Multi-binding devices, shared devices, and pure Bluetooth devices do not support adding to automation" duration:1 position:CSToastPositionCenter];
        return;
    }
