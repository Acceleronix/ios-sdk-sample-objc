...
- (void)viewDidLoad {
    ...
    [addButton setTitle:@"Add Scene" forState:UIControlStateNormal];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [actionButton setTitle:@"Execute Scene" forState:UIControlStateNormal];
...
- (void)actionButtonClick:(UIButton *)sender {
    ...
    [QuecSceneService.sharedInstance executeSceneWithFid:model.fid sceneId:model.sceneInfo.sceneId success:^(QuecActionExecuteResultModel * _Nonnull executeResultModel) {
        ...
        if (executeResultModel.executeResult) {
            [self.view makeToast:@"Execution Successful" duration:1 position:CSToastPositionCenter];
        }else {
            [self.view makeToast:@"Execution Failed" duration:1 position:CSToastPositionCenter];
        }
